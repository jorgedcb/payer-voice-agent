...
async def test_backend_payload_renders_both_agents():
    ...
    assert "W as in whiskey, one, two, three, four, five, six, seven, eight, nine" in interview
...
def test_backend_network_scope_keeps_its_meaning(scope, network_first, both, stop_when_oon):
    spec = prepare(payload(network_benefit_scope=scope))
    rendered = prompts.instructions("verification", spec)
    first_topic = "1. Provider network status" if network_first else "1. Benefits for physical therapy"
...
@pytest.mark.parametrize(
    "both,oon,scope",
    [(True, True, "both_networks"), (False, True, "provider_network"), (False, False, "in_network_only"), (True, False, "in_network_only")],
)
def test_the_retired_two_flag_shape_is_still_accepted(both, oon, scope):
    # A job dispatched by the previous backend build can still arrive while
    # this worker is live: the two flags translate to the scope they meant.
    spec = prepare(payload(require_both_network_benefits=both, collect_oon_benefits=oon))
    assert spec.network_benefit_scope == scope


def test_an_explicit_scope_outranks_the_retired_flags():
    spec = prepare(payload(require_both_network_benefits=True, network_benefit_scope="in_network_only"))
    assert spec.network_benefit_scope == "in_network_only"
...
async def test_handoff_uses_the_same_spec_with_phase_specific_formatting():
    ...
    assert "W as in whiskey, one, two, three, four, five, six, seven, eight, nine" in interview.instructions
...
@pytest.mark.parametrize("service_type,label", [
...
def test_both_prompts_derive_the_service_label(service_type, label):
    ...
    assert f"benefits and eligibility questions for {label}." in prompts.instructions("navigator", spec)
...
@pytest.mark.parametrize("service,diagnosis,ask_limits,expected", [
...
def test_visit_limit_followup_is_derived_from_call_facts(service, diagnosis, ask_limits, expected):
...
@pytest.mark.parametrize("codes,spoken", [
    (["0362T", "0373T"],
