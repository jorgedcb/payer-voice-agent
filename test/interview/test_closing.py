...
@pytest.mark.parametrize("farewell", [
    pytest.param("Thank you for calling Aetna, and happy weekend. Bye bye.", id="recorded-goodbye"),
    pytest.param("The next team will help you. Thanks for calling, goodbye.", id="handoff-goodbye"),
    pytest.param("Your reference for my part of the call is AB73921. "
                 "Please stay on the line for the next team. Goodbye.", id="reference-before-transfer"),
])
async def test_representative_goodbye_during_transfer_does_not_start_closing(
    session, start_assistant, llm, farewell,
):
    ...
    await replay(agent, [
        ("user", "Hello. Good afternoon. Thank you for calling Aetna. My name is Jamie, "
         "and I'll be your customer at Rocket. Please be advised there will be a quick "
         "survey after this call. May I help you today?"),
        ("assistant", "Hi Jamie, I'm calling to check benefits for one of your members."),
        ("user", "I'll be more than happy to assist you check benefits and eligibility. "
         "And can I have your name and the provider such as"),
        ("assistant", "My name is Annie, S for sierra. The provider is Sample Therapy Center."),
        ("user", "Thank you. And can I also have the address of the provider piece?"),
        ("assistant", "Sure. The address is 123 Sample Street, Austin, Texas, 78701."),
        ("user", "Thank you for providing the provider's information, um, Annie. "
         "I was able to pull up the provider. And are you calling for the member "
         "named Sample Member?"),
        ("assistant", "Yes, the member is Sample Member."),
        ("user", "Okay. Perfect. I already validated the member's account. Sorry. "
         "Can you confirm the date of birth, please?"),
        ("assistant", "Yes, the date of birth is January first, nineteen ninety."),
        ("user", "Thank you. I already validated the member's account. And let me go "
         "check if he handled this member. for a while."),
        ("assistant", "Okay."),
        ("user", "Thank you. And as per checking here, Annie,"),
        ("assistant", "Okay."),
        ("user", "Uh, this type of members plan is going to be handled by our "
         "dedicated department, and I can only help you by transferring you to them. "
         "Is it okay with you?"),
        ("assistant", "Yes, that's okay."),
        ("user", "Okay. Perfect. I'll please stay on the line, and I'll be transferring you now."),
    ])
    ...
    user_input = "Hello, this is Morgan with the dedicated benefits team. How can I help you?"
    result = await session.run(user_input=user_input)
    show_response(user_input, result)
    assert not called(result, "end_call")
    await result.expect.contains_message(role="assistant").judge(
        llm, intent="Starts or resumes a benefits inquiry with the new representative, "
...
@pytest.mark.asyncio
async def test_failed_transfer_with_no_way_to_continue_starts_closing(session, start_assistant):
    agent = await start_assistant()
    await replay(agent, [
        ("user", "My name is Jamie R. This plan is handled by our dedicated department. "
         "Can I transfer you?"),
        ("assistant", "Yes, please."),
        ("user", "Please stay on the line while I transfer you."),
    ])
    user_input = "I'm sorry, the transfer failed and that department is closed. " \
        "I cannot verify these benefits or connect you to anyone today. " \
        "Please call back on Monday. Goodbye."
    result = await session.run(user_input=user_input)
    show_response(user_input, result)
    assert called(result, "end_call"), "An explicitly failed transfer must not leave the agent waiting."
