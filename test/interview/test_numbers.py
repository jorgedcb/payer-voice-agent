...
@pytest.mark.parametrize("question,expected", [
    ("What's a good callback number for you, Annie?",
     "2025550123"),
    ("What is the provider's NPI?",
     "1477583926"),
    ("What's that member's ID number?",
     "W812640935"),
])
async def test_identifiers_are_paced_one_character_at_a_time_on_first_read(
    session, start_assistant, llm, question, expected,
):
    await start_assistant(
        caller_first_name="Annie", callback_number="+12025550123",
        npi="1477583926", member_id="W812640935",
    )
    ...
    await message.judge(
        llm, intent=f"Reads the complete identifier {expected} one character at a time, preserving every digit and letter in order, as plain spoken words with no markup or tags. Digits may be written numerically or as words, and letters may be spoken directly or phonetically. Wording and punctuation may vary."
    )
    result.expect.no_more_events()


@pytest.mark.parametrize("dob,spoken_date", [
...
async def test_date_of_birth_is_read_naturally(
    session, start_assistant, llm, dob, spoken_date,
):
...
async def test_caller_name_is_spelled_with_the_phonetic_alphabet(
    ...
    assert text.index("alpha") < text.index("november") < text.index("india") < text.index("echo")
    result.expect.no_more_events()
