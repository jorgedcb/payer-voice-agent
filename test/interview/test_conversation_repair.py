...
HISTORY = [
    ...
    ("user", "For eligibility, benefits, or claims, press one. For authorizations, press two."),
    (
        "user",
...
async def test_apology_after_interruption_does_not_reverse_caller_role(
    ...
    agent = await start_assistant(
        caller_first_name="Annie", caller_last_initial="S", callback_number="2025550123",
        service_type="mental_health", cpt_codes=["97151", "97152", "97153"],
    )
...
async def test_harmless_transcription_variation_does_not_derail_the_question(
    ...
    agent = await start_assistant(
        caller_first_name="Annie", service_type="physical_therapy",
        cpt_codes=["97110"], service_locations=["11"],
    )
    await replay(agent, [
        ("user", "What is your name and what are you calling about?"),
        ("assistant", "My name is Annie. I'm calling to verify physical therapy benefits."),
    ])
...
async def test_casual_thanks_does_not_trigger_unsolicited_name_correction(
    session, start_assistant, llm, heard_name,
):
    agent = await start_assistant(
        caller_first_name="Annie", caller_last_initial="S", callback_number="2025550123",
        service_type="mental_health", cpt_codes=["97151", "97152", "97153"],
