...
@pytest.mark.parametrize('destination', ['https://cloud.langfuse.com', 'https://hipaa.cloud.langfuse.com.evil.test', 'http://hipaa.cloud.langfuse.com', 'https://hipaa.cloud.langfuse.com/path'])
def test_unapproved_trace_destination_never_constructs_exporter(monkeypatch, destination, caplog):
    ...
    provider = Mock()
    monkeypatch.setattr(agent, 'TracerProvider', provider)
    assert agent._setup_langfuse(session_id='vc-synthetic-call') is None
...
async def test_dial_error_logs_and_shutdown_omit_provider_details(monkeypatch, caplog):
    ...
    ctx = SimpleNamespace(room=SimpleNamespace(name='synthetic-call'), shutdown=Mock(), api=SimpleNamespace(sip=SimpleNamespace(create_sip_participant=AsyncMock(side_effect=error))))
...
async def test_session_start_log_omits_caller_and_facility(monkeypatch, caplog):
    ...
    class PipelineReached(Exception):
        pass
    monkeypatch.setattr(agent, '_setup_langfuse', lambda **kwargs: None)
    ...
    ctx = SimpleNamespace(is_fake_job=lambda: True, job=SimpleNamespace(metadata=''), room=SimpleNamespace(name='synthetic-call'))
...
def test_approved_exporter_pins_destination_over_ambient_otel(monkeypatch):
    ...
    monkeypatch.setenv("LANGFUSE_PUBLIC_KEY", "synthetic-public")
    monkeypatch.setenv("LANGFUSE_SECRET_KEY", "synthetic-secret")
    monkeypatch.setenv("OTEL_EXPORTER_OTLP_TRACES_ENDPOINT", "https://unapproved.example/traces")
    exporter = Mock()
    ...
    monkeypatch.setattr(agent, "TracerProvider", Mock())
    monkeypatch.setattr(agent, "set_tracer_provider", Mock())
    assert agent._setup_langfuse(session_id="vc-synthetic-call") is not None
    assert exporter.call_args.kwargs["endpoint"] == "https://hipaa.cloud.langfuse.com/api/public/otel/v1/traces"
    assert exporter.call_args.kwargs["headers"]["x-langfuse-ingestion-version"] == "4"


@pytest.mark.parametrize(('code', 'sip', 'expected'), [
    ('unavailable', '486', 'sip_486'),
    ('not_found', None, 'not_found'),
    ('synthetic patient secret', None, 'unknown'),
    ('unavailable', '12025550100', 'unavailable'),
])
async def test_dial_retains_only_validated_protocol_codes(monkeypatch, caplog, code, sip, expected):
    monkeypatch.setenv('SIP_TRUNK_ID', 'synthetic-trunk')
    error = api.SipCallError(code, 'synthetic patient secret', status=503,
        metadata={'sip_status_code': sip} if sip else {})
    ctx = SimpleNamespace(room=SimpleNamespace(name='synthetic-call'), shutdown=Mock(),
        api=SimpleNamespace(sip=SimpleNamespace(create_sip_participant=AsyncMock(side_effect=error))))
...
def test_spans_carry_call_id_as_langfuse_session_inside_a_job(monkeypatch, restore_livekit_tracer):
    ...
    assert hasattr(lk_traces, "_job_stamp_attributes"), "livekit-agents changed its job telemetry internals"
    monkeypatch.setattr(lk_traces, "_job_stamp_attributes", lambda: {"room_id": "RM_synthetic"})

    provider = agent._setup_langfuse(session_id="vc-synthetic-call")
    ...
    assert span.resource.attributes["service.name"] == agent.TRACING_SERVICE_NAME
