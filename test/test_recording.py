...
def recording_ctx(monkeypatch):
    ...
    egress = SimpleNamespace(
        ...
        stop_egress=AsyncMock(return_value=api.EgressInfo(egress_id='EG_test', status=api.EgressStatus.EGRESS_COMPLETE)),
        list_egress=AsyncMock(),
    )
    callbacks = []
    return SimpleNamespace(
        is_fake_job=lambda: False, room=SimpleNamespace(name='call-123'),
        api=SimpleNamespace(egress=egress), add_shutdown_callback=callbacks.append,
        callbacks=callbacks,
    )
...
async def test_shutdown_leaves_upload_completion_to_the_backend(recording_ctx, monkeypatch):
    ctx = recording_ctx
    ctx.api.egress.stop_egress.return_value = api.EgressInfo(status=api.EgressStatus.EGRESS_ENDING)
    ctx.api.egress.list_egress.return_value = api.ListEgressResponse(items=[api.EgressInfo(status=api.EgressStatus.EGRESS_COMPLETE)])
...
async def test_recording_errors_are_contained(recording_ctx, caplog, phase):
    ctx = recording_ctx
    if phase == 'start':
        ctx.api.egress.start_room_composite_egress.side_effect = RuntimeError('sensitive details')
    else:
