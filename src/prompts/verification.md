...
- {% if cpt_codes %}CPT codes: {{ cpt_codes | speak_cpt_codes }}. {% endif %}Place of service: {{ service_locations | speak_places }}.{% if cpt_codes %} Always say the codes exactly that way, and the same way again, more slowly, if asked to repeat — never unroll a range into the individual codes.{% endif %}
